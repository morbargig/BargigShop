...
class Admin extends Component {
    ...
    render() {
        ...
        return <div>  <h5>{this.props.state.user ? 'Hello Admin ' + this.props.state.user.email : null} </h5>
            ...
            {this.state.numberInputs.map(i =>  <label>{i} <input type='number' autocomplete="off" placeholder={i} pattern="\d*" className="numbers-onl" maxlength="4" name={i} value={this.state.newItem[i]} onChange={this.updateItemdetails} /> </label> )}
