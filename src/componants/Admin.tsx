...
class Admin extends Component<any> {
    ...
    addImageByColor = () => {
        ...
        let me = this
        this.setState({ newItem, color: "" }, function () { console.log(me.state) })
    ...
    render() {
        ...
        return <div>  <h5>{this.props.state.user ? 'Hello Admin ' + this.props.state.user.email : null} </h5>
            ...
            {this.state.stringInputs.map((i: any) => <label>{i} <input type='text' placeholder={i} name={i} value={this.state.newItem[i]} onChange={this.updateItemdetails} /> </label>)}
            <br></br>
            {/* <br></br> */}
            <br></br>
            ...
            {/* @ts-ignore */}
            {this.state.numberInputs.map((i: any) => <label>{i} <input type='number' autocomplete="off" placeholder={i} pattern="\d*" min="1" max="999" name={i} value={this.state.newItem[i]} onChange={this.updateItemdetails} /> </label>)}
            ...
            <br></br>
            {this.state.inputsWithFewInputs.map((i: any) => <label id={i}>** אחד אחרי השני  {i} אתה צריך להוסיף **
                <div>
                    {i} : <datalist id={`search${i}`} className='select-input' onChange={this.updateItemdetails}>
                        {this.state[i + 'List'].map((c: any) => <option className={`options${c}`} value={c}>{c}  </option>)}
                    </datalist>
            ...
            <div>
                צבע מוצר : <datalist id="searchColor" className='select-input' onChange={this.updateItemdetails}>
                    {this.state.colorSData.map((c: any) => <option className="optionsColor" value={c}>{c}  </option>)}
                </datalist>
            ...
            <br></br>
            {/* @ts-ignore */}
            <button className="waves-effect waves-dark btn" onClick={this.addNewitem} >  לחץ עליי להעלות מוצר רק כאשר כל הפרטים תקינים </button>
            <br></br>
            <br></br>
